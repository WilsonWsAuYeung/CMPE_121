    ...
    for(;;)
    {
        ...
        */
        ...
        if (0u != USBUART_GetConfiguration())
        {
            ...
            if (USBUART_EVENT_PENDING == USBUART_GetEPState(3)) { 
                            ...
                            if(buff1or2 % 2 == 0){
                                for(i = 0; i < 64; i++){
                                    if(Lcount < leftOver){
                                        ...
                                        RxBuffer0[i] = TempStore[i+(BufferMultiplier*BufferRuns)-leftOver];
                                    }
                                }
                                while(USBUART_IN_BUFFER_EMPTY != USBUART_GetEPState(2)){
                                ...
                                for(i = 0; i < 64; i++){
                                    if(Lcount < leftOver){
                                        RxBuffer1[i] = remainder[i];
                                        Lcount++;
                                    }
                                    else{
                                        RxBuffer1[i] = TempStore[i+(BufferMultiplier*BufferRuns)-leftOver];
                                    }
                                }
                                
                               //if (USBUART_IN_BUFFER_EMPTY == USBUART_GetEPState(2)){
                                 while(USBUART_IN_BUFFER_EMPTY != USBUART_GetEPState(2)){
                                 
                                }
                                 USBUART_LoadInEP(2, RxBuffer1, USBUART_BUFFER_SIZE);  
                                while (0u == USBUART_CDCIsReady())
                                {
                                }   
